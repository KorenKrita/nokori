...
- **クロスツール観測性**：Claude Code と Cursor の両方に対応。`nokori test`、`status`、`health`、`logs`、`extract`、`maintain`、Web UI で、ルールがなぜ発火したか/しなかったかを確認できる。
...
Nokori のホットパスは 1 本で、各ランタイムをそこへ写像する。Claude Code / Cursor は既存の Python hook を直接呼び、OMP は `~/.omp/agent/extensions/nokori.ts` の TypeScript ブリッジから同じディスパッチャへイベントを渡す。検索、Gate マーカー、job、ルール保存先はそのまま `~/.nokori/` 配下で、抽出時だけ `~/.omp/agent/sessions/**/*.jsonl` の現在セッション JSONL をローカルから読む。

| Claude Code / Cursor | OMP | やること | レイテンシ予算 |
|----------------------|-----|---------|---------------|
...
| `SessionEnd` | `session_shutdown` | セッション終了：OMP の session manager が示す現在セッションファイルから抽出 job を作り、async モードならそのローカル JSONL に対して抽出を走らせる | <= 200ms |
...
OMP では、`session_start` は `pi.sendMessage(...)` を使う。これは lifecycle handler に戻り値ベースの注入チャネルがないため。`before_agent_start` は `message` を返す。こちらは `BeforeAgentStartEventResult.message` が型付きの注入チャネルだからである。Bridge の timeout 値は runtime budget であり、`session_shutdown` は意図的に短い 2s の teardown budget を保つ。

---

## 注入 vs 阻断

| | 注入（`additionalContext` / OMP ブリッジ経由の注入メッセージ） | Gate（PreToolUse deny / OMP の tool block） |
|--|------------------------------|-------------------------|
...
| 用語 | 説明 |
|----|------|
| **hook** | Claude Code / Cursor が決まったタイミングで自動実行する短いコマンド |
