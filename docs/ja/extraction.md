...
セッション終了後に実行されるコールドパス。対話のホットパスには乗らない。LLM を設定しておけば、Nokori はそのセッションの transcript を読み、候補ルールを抽出し、コールドパスパイプラインに通す。Claude Code・Cursor・Pi・OMP は同じ抽出器を共有し、Pi / OMP では TypeScript ブリッジが `session_shutdown` を受けてランタイムの session manager から現在のセッションファイルを取り出し、そのローカル JSONL を既存の Python ディスパッチャへ渡す。Pi は `reason: "reload"` のイベントを無視するため、`/reload` で抽出が早期キューイングされることはない。
...
1. **読み込み**：transcript を読む。単一ファイル上限 50MB。Pi / OMP では `session_shutdown` が session manager 経由で現在の `~/.pi/agent/sessions/**/*.jsonl` または `~/.omp/agent/sessions/**/*.jsonl` を渡す
