...
关会话后运行，不在交互热路径上。配置 LLM 后，Nokori 读取该场对话的 transcript，提取可能的规则，再让每条候选走完冷路径飞轮。Claude Code、Cursor、Pi 与 OMP 走同一条提取管线；在 Pi / OMP 中，已安装的 TypeScript bridge 会在 `session_shutdown` 时通过 runtime session manager 取到当前 session 文件，并把本地 JSONL 路径交给现有 Python dispatcher。Pi 会忽略 `reason: "reload"` 的事件，因此 `/reload` 不会提前排队提取。
...
1. **读** transcript，单文件上限 50MB
   ...
   Pi / OMP session 日志分别位于 `~/.pi/agent/sessions/**/*.jsonl` 与 `~/.omp/agent/sessions/**/*.jsonl`；由 `session_shutdown` 触发后，bridge 会通过 session manager 把当前文件交给同一套压缩 / 提取流程。
