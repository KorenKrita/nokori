...
def describe_omp_extension() -> dict[str, object]:
    ...
    managed = _is_managed_omp_source(source)
    current = managed and source == _build_omp_hook_source()
    if current:
        note = ""
    elif managed:
        note = "generated extension is stale; rerun `nokori install --omp`"
    else:
...
def run(args: argparse.Namespace, cfg: Config) -> int:
    ...
    managing_omp = omp_target and not args.disable and not args.enable
    if managing_omp and omp_before is not None and not _is_managed_omp_source(omp_before):
        action = "remove" if args.uninstall else "overwrite"
        print(
            f"nokori: refusing to {action} unmanaged OMP extension: {omp_path}\n"
