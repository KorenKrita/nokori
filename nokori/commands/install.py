...
OMP_GENERATED_MARKER = "// @generated by nokori install --omp;"
PI_GENERATED_MARKER = "// @generated by nokori install --pi;"
...
def resolve_install_targets(
    ...
    *,
    ...
    """Default: Claude only. --all: Claude+Cursor. Pi/OMP stay explicit. Uninstall no flags: all."""
    all_native = bool(getattr(args, "all_platforms", False))
    claude = all_native or bool(getattr(args, "claude", False))
    cursor = all_native or bool(getattr(args, "cursor", False))
    pi = bool(getattr(args, "pi", False))
    omp = bool(getattr(args, "omp", False))
    if not claude and not cursor and not pi and not omp:
        if uninstall:
            return True, True, True, True
        return True, False, False, False
    return claude, cursor, pi, omp
...
def _pi_extension_path() -> Path:
    base = os.environ.get("NOKORI_PI_HOME") or os.environ.get("PI_CODING_AGENT_DIR")
    if base:
        return Path(base).expanduser() / "extensions" / "nokori.ts"
    return Path("~/.pi/agent/extensions/nokori.ts").expanduser()
...
def _build_omp_hook_source() -> str:
    ...
    return textwrap.dedent(
        f"""\
        {OMP_GENERATED_MARKER} do not edit manually.
...
def _replace_source_exact(source: str, old: str, new: str, *, expected: int = 1) -> str:
    """Apply a checked source-template delta so bridge API drift fails loudly in tests/install."""
    actual = source.count(old)
    if actual != expected:
        raise RuntimeError(
            f"extension template mismatch: expected {expected} occurrence(s), found {actual}"
        )
    return source.replace(old, new)


def _build_pi_hook_source() -> str:
    """Generate the Pi bridge from the source-compatible OMP template plus Pi-specific deltas."""
    source = _build_omp_hook_source()
    source = _replace_source_exact(source, OMP_GENERATED_MARKER, PI_GENERATED_MARKER)
    source = _replace_source_exact(
        source,
        'import type { ExtensionAPI } from "@oh-my-pi/pi-coding-agent/extensibility/extensions";',
        'import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";',
    )
    source = _replace_source_exact(source, 'host: "omp"', 'host: "pi"')
    source = _replace_source_exact(
        source,
        'env: { ...process.env, NOKORI_HOST: "omp" },',
        'env: { ...process.env, NOKORI_HOST: "pi" },',
    )
    source = _replace_source_exact(
        source,
        "function getSessionFile(ctx: {\n",
    ...
    source = _replace_source_exact(source, '      attribution: "agent",\n', "", expected=2)
    source = _replace_source_exact(
        source,
        '  pi.on("session_start", (_event, ctx) => {\n',
...
def _is_managed_pi_source(source: str | None) -> bool:
    return source is not None and source.startswith(PI_GENERATED_MARKER)


def _is_managed_omp_source(source: str | None) -> bool:
    return source is not None and source.startswith(OMP_GENERATED_MARKER)
...
def _describe_extension(
    *,
    ...
    managed_check: Callable[[str | None], bool],
    install_flag: str,
) -> dict[str, object]:
    try:
    ...
    managed = managed_check(source)
    current = managed and source == source_builder()
    if current:
        note = ""
    elif managed:
        note = f"generated extension is stale; rerun `nokori install {install_flag}`"
    else:
        note = "existing file is not managed by Nokori"
    return {
        "path": str(path),
        "installed": managed,
        "managed": managed,
...
def describe_pi_extension() -> dict[str, object]:
    """Installed state for the generated Pi extension bridge."""
    return _describe_extension(
        path=_pi_extension_path(),
        source_builder=_build_pi_hook_source,
        managed_check=_is_managed_pi_source,
        install_flag="--pi",
    )


def describe_omp_extension() -> dict[str, object]:
    """Installed state for the generated OMP extension bridge."""
    return _describe_extension(
        path=_omp_extension_path(),
        source_builder=_build_omp_hook_source,
        managed_check=_is_managed_omp_source,
        install_flag="--omp",
    )
...
def _write_text_file(path: Path, data: str) -> None:
    try:
        atomic_write_text(path, data, mkdir=True)
    except OSError as e:
        raise ValueError(f"cannot write {path}: {e}") from e
...
def run(args: argparse.Namespace, cfg: Config) -> int:
    claude_target, cursor_target, pi_target, omp_target = resolve_install_targets(
        args,
        uninstall=bool(args.uninstall),
    )
    command = _build_command()
    pi_source = _build_pi_hook_source() if pi_target else ""
    omp_source = _build_omp_hook_source() if omp_target else ""

    claude_path = _settings_path()
    cursor_path = _cursor_hooks_path()
    pi_path = _pi_extension_path()
    ...
    try:
        ...
        pi_before = _read_text_file(pi_path) if pi_target else None
    ...
    managed_extensions = (
        ("Pi", pi_target, pi_before, pi_path, _is_managed_pi_source),
        ("OMP", omp_target, omp_before, omp_path, _is_managed_omp_source),
    )
    for label, target, before, path, managed_check in managed_extensions:
        managing = target and not args.disable and not args.enable
        if managing and before is not None and not managed_check(before):
            action = "remove" if args.uninstall else "overwrite"
            print(
                f"nokori: refusing to {action} unmanaged {label} extension: {path}\n"
                f"  Move or rename that file, then retry.",
                file=sys.stderr,
            )
            return 1
    ...
    if args.uninstall:
        ...
        pi_after = None if pi_target else pi_before
        ...
        pi_after = pi_source if pi_target else pi_before
