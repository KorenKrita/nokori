...
- confidence: high (user repeated/emphasized strongly — "永远不要"/"必须"/"always"/"never again", or lesson is universally applicable across projects) | medium (user corrected once clearly, lesson is reusable but may be context-dependent) | low (inferred from failure→fix pattern, or user's correction was mild/ambiguous). Never high if only assistant self-fixed without user pushback. Do NOT emit rules with low confidence.
...
3) confidence: high only with strong emphasis/repetition; medium for clear single correction; low = drop (do not emit).
