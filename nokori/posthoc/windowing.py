...
class TranscriptTurn(TypedDict):
    """Minimal turn shape used by posthoc windowing."""
...
# Accept plain dicts from transcript readers (list invariance vs TypedDict).
TurnMapping = Mapping[str, object]
...
def compute_event_window(
    ...
    window_turns: list[dict[str, object]] = [dict(turns[start_idx])]
    injection_tokens = _tokenize(str(turns[start_idx].get("content", "") or ""))

    # Pre-compute injection embedding if embedding_fn available
    injection_embedding: list[float] | None = None
    if embedding_fn is not None:
        injection_content = str(turns[start_idx].get("content", "") or "")
...
def extract_window_content(window_turns: Sequence[TurnMapping]) -> str:
...
def _is_topic_shift(
    ...
    window_so_far: Sequence[TurnMapping],
    ...
    *,
    ...
    if len(window_so_far) >= _LARGE_TURN_GAP:
        recent_content = " ".join(str(t.get("content", "") or "") for t in window_so_far[-3:])
    ...
    tool_name = turn.get("tool_name")
    if rule_tool_tags and isinstance(tool_name, str) and tool_name:
...
def _preceded_by_tool_sequence(turns: Sequence[TurnMapping], current_idx: int) -> bool:
