"""Detect Claude Code, Cursor, Pi, or OMP from transcript / session log paths."""
...
def detect_host_from_payload(payload: dict) -> Host:
    ...
    explicit Pi/OMP payload or env. Cursor and Claude precedence stays intact.
    ...
    if isinstance(payload_host, str):
        normalized_host = payload_host.strip().lower()
        if normalized_host == Host.PI.value:
            return Host.PI
        if normalized_host == Host.OMP.value:
            return Host.OMP

    env_host = os.environ.get("NOKORI_HOST", "").strip().lower()
    if env_host == Host.PI.value:
        return Host.PI
    if env_host == Host.OMP.value:
        return Host.OMP
...
def effective_gate_matcher(base_matcher: str, host: Host) -> str:
    ...
    if host == Host.PI and base_matcher == DEFAULT_GATE_MATCHER:
        return PI_GATE_MATCHER
