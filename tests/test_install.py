...
def test_install_pi_dry_run_shows_pi_specific_bridge(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "pi-agent"
    ext = pi_home / "extensions" / "nokori.ts"

    r = _run(
        "install", "--pi", "--dry-run",
        env_extra={
            "NOKORI_DATA_DIR": str(data),
            "NOKORI_PI_HOME": str(pi_home),
        },
    )

    assert r.returncode == 0, r.stderr
    assert str(ext) in r.stdout
    assert 'import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";' in r.stdout
    assert 'host: "pi"' in r.stdout
    assert 'env: { ...process.env, NOKORI_HOST: "pi" }' in r.stdout
    assert "getSessionId" in r.stdout
    assert 'payload.session_id = sessionId;' in r.stdout
    assert 'payload.transcript_path = sessionFile;' in r.stdout
    assert r.stdout.count('if (event.reason === "reload") return;') == 2
    assert 'attribution: "agent"' not in r.stdout
    assert not ext.exists()


def test_install_pi_respects_pi_coding_agent_dir(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "custom-pi-agent"
    ...
    assert r.returncode == 0, r.stderr
    assert (pi_home / "extensions" / "nokori.ts").is_file()


def test_install_all_can_be_combined_with_explicit_pi_and_omp(tmp_path):
    data = tmp_path / "data"
    claude_home = tmp_path / "claude"
    cursor_home = tmp_path / "cursor"
    pi_home = tmp_path / "pi-agent"
    ...
    assert (pi_home / "extensions" / "nokori.ts").is_file()
    assert (omp_home / "extensions" / "nokori.ts").is_file()
    targets = json.loads((data / "install_targets.json").read_text())
    assert targets["platforms"] == ["claude", "cursor", "omp", "pi"]


def test_install_pi_writes_idempotent_extension_and_records_platform(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "pi-agent"
    env = {
        "NOKORI_DATA_DIR": str(data),
    ...
    first = _run("install", "--pi", env_extra=env)
    assert first.returncode == 0, first.stderr
    assert "Recorded install platforms: pi" in first.stdout

    ext = pi_home / "extensions" / "nokori.ts"
    source = ext.read_text()
    assert source.startswith("// @generated by nokori install --pi; do not edit manually.\n")
    assert 'import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";' in source
    assert source.count('pi.on("tool_call"') == 1
    assert source.count('if (event.reason === "reload") return;') == 2

    second = _run("install", "--pi", env_extra=env)
    assert second.returncode == 0, second.stderr
    assert "no changes needed" in second.stdout
    assert ext.read_text() == source
    targets = json.loads((data / "install_targets.json").read_text())
    assert targets["platforms"] == ["pi"]


def test_install_pi_refuses_to_overwrite_or_remove_unmanaged_extension(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "pi-agent"
    ext = pi_home / "extensions" / "nokori.ts"
    ext.parent.mkdir(parents=True)
    custom = "export default function customPiExtension() {}\n"
    ext.write_text(custom)
    env = {
        "NOKORI_DATA_DIR": str(data),
        "NOKORI_PI_HOME": str(pi_home),
    }

    install = _run("install", "--pi", env_extra=env)
    assert install.returncode == 1
    assert "refusing to overwrite unmanaged Pi extension" in install.stderr
    assert ext.read_text() == custom

    uninstall = _run("install", "--uninstall", "--pi", env_extra=env)
    assert uninstall.returncode == 1
    assert "refusing to remove unmanaged Pi extension" in uninstall.stderr
    assert ext.read_text() == custom


def test_uninstall_pi_removes_managed_extension(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "pi-agent"
    env = {
        "NOKORI_DATA_DIR": str(data),
    ...
    installed = _run("install", "--pi", env_extra=env)
    assert installed.returncode == 0, installed.stderr

    ext = pi_home / "extensions" / "nokori.ts"
    removed = _run("install", "--uninstall", "--pi", env_extra=env)
...
def test_health_reports_pi_bridge_and_warns_when_stale(tmp_path):
    data = tmp_path / "data"
    pi_home = tmp_path / "pi-agent"
    env = {
        "NOKORI_DATA_DIR": str(data),
    ...
    healthy = _run("health", env_extra=env)
    assert healthy.returncode == 0, healthy.stderr
    assert "hooks.pi" in healthy.stdout
    assert "registered" in healthy.stdout

    ext = pi_home / "extensions" / "nokori.ts"
    ext.write_text(ext.read_text() + "// stale\n")
    stale = _run("health", env_extra=env)
    assert stale.returncode == 0, stale.stderr
    assert "hooks.pi" in stale.stdout
    assert "stale" in stale.stdout
    assert "nokori install --pi" in stale.stdout
