...
class TestLifecycle:
    ...
    def test_exit_without_db_access_no_close(self, ctx_env):
        ...
        with patch("nokori.hooks.context.open_db") as mock_open:
            with HotPathContext(payload, cfg, host=Host.CLAUDE):
                pass
...
class TestDbOpenFailure:
    def test_db_returns_none_on_failure(self, ctx_env):
        payload, cfg = ctx_env
        with patch("nokori.hooks.context.open_db", side_effect=DbError("locked")):
            with HotPathContext(payload, cfg, host=Host.CLAUDE) as ctx:
                assert ctx.db is None
    ...
    def test_error_added_on_db_failure(self, ctx_env):
        payload, cfg = ctx_env
        with patch("nokori.hooks.context.open_db", side_effect=DbError("timeout")):
            with HotPathContext(payload, cfg, host=Host.CLAUDE) as ctx:
                _ = ctx.db
    ...
    def test_db_returns_none_on_non_db_error(self, ctx_env):
        payload, cfg = ctx_env
        with patch("nokori.hooks.context.open_db", side_effect=OSError("permission denied")):
            with HotPathContext(payload, cfg, host=Host.CLAUDE) as ctx:
                assert ctx.db is None
                assert ctx.db_unavailable is True
...
class TestRecordEvent:
    def test_writes_event_when_db_available(self, ctx_env):
        ...
        mock_db = MagicMock()
        with patch("nokori.hooks.context.open_db", return_value=mock_db):
            with patch("nokori.hooks.context.write_event", return_value="evt-123") as mock_write:
                with HotPathContext(payload, cfg, host=Host.CLAUDE, session_id="s1") as ctx:
    ...
    def test_returns_none_when_db_unavailable(self, ctx_env):
        payload, cfg = ctx_env
        with patch("nokori.hooks.context.open_db", side_effect=DbError("nope")):
            with HotPathContext(payload, cfg, host=Host.CLAUDE) as ctx:
    ...
    def test_returns_none_when_write_event_raises(self, ctx_env):
        ...
        mock_db = MagicMock()
        with patch("nokori.hooks.context.open_db", return_value=mock_db):
            with patch("nokori.hooks.context.write_event", side_effect=Exception("write fail")):
                with HotPathContext(payload, cfg, host=Host.CLAUDE) as ctx:
    ...
    def test_prompt_snippet_forwarded(self, ctx_env):
        ...
        mock_db = MagicMock()
        with patch("nokori.hooks.context.open_db", return_value=mock_db):
            with patch("nokori.hooks.context.write_event") as mock_write:
                with HotPathContext(payload, cfg, host=Host.CLAUDE, session_id="s2") as ctx:
