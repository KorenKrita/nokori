...
| **ハイブリッド検索** | BM25 は常に使える。任意で remote embedding またはローカル Granite 多言語モデルを足して意味検索を補い、RRF で融合し、v6 applicability と MMR 風 selection が HOT/WARM を決める。 |
...
| **candidate / active / trusted / suppressed / archived** | v6 ライフサイクル：候補、注入可能、信頼済み、shadow recovery、終端 archive |
...
3. **抽出する**：extractor role が concepts、required concept groups、variants、excluded contexts、evidence quotes、source metadata を持つ構造化 v6 candidates を出す
...
| 危険または低信頼の merge | 保守的に keep_both または reject_new。trusted replacement にはより高い v6 の基準が必要 |
...
|------|-----------|-----------|----------|
| `candidate` | いいえ。shadow / evidence のみ | いいえ | `nokori add` または cold extraction が作る構造化 v6 candidate |
...
- **手動 `nokori add` は常に `candidate` を作る**。v6 の structured trigger concepts/groups は入るが、`--confidence high --source-type correction` でも lifecycle は飛ばせない。
...
- **Lifecycle transitions**（毎日）：posthoc/shadow evidence が v6 control law に従って candidate、active、trusted、suppressed を更新する
...
| 階層 | 入る条件 | 注入内容 |
|------|---------|----------|
| HOT | v6 applicability を通った `active`/`trusted` result で utility が正。通常は最大 1 件、2 件目は domain / concept set が明確に別で trigger evidence も強い場合だけ | trigger + action + rationale |
