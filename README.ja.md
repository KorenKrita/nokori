...
<p align="center">
  <img src="docs/assets/nokori-one-pager.ja.png" width="880" alt="Nokori プロジェクト一枚紙" />
</p>
...
<p align="center">
  <img src="docs/assets/readme-illustrations/01-corrections-become-rules.ja.png" width="880" alt="訂正がローカルの永続ルールに沈殿する" />
  <br />
  <sub>一つ一つの訂正が永続的なローカルルールに沈殿する。</sub>
</p>
...
**前提条件**：Python >= 3.11、Claude Code・Cursor・Pi・OMP のいずれかがインストール済み

```bash
# 推奨：uv でインストール（ローカル意味検索込み）
uv tool install "nokori[local-embed]"

# Hook / bridge を登録
nokori install --pi         # Pi のみ
nokori install --omp        # OMP のみ
# --all は Claude Code + Cursor、--cursor は Cursor、デフォルトは Claude Code のみ

# 動作確認（Pi / OMP 導入時は hooks.pi / hooks.omp が表示される）
nokori health
```

Pi と OMP では、recall は `before_agent_start`、Gate は `tool_call`、セッション後の抽出は `session_shutdown` で動く。Pi の `/reload` は無視されるため、Nokori が現在のセッションを終了扱いすることはない。
...
> 詳しいインストールガイド（Claude Code / Cursor / Pi / OMP 設定、更新、アンインストール等）は[インストール文書](docs/ja/installation.md)を参照
...
普段どおり Claude Code / Cursor / Pi / OMP でコードを書けばよい。ルールにマッチすると、Agent の返信前にリマインドが注入される。`trusted` + `gate_eligible` のルールは、最初の敏感なツール呼び出しを一度差し止める。
...
<table>
  ...
  <tr>
    <td width="50%">
      ...
      Claude Code と Cursor をネイティブにサポートし、Pi と OMP は小さな TypeScript ブリッジで同じ Python ディスパッチャへ接続する。
    </td>
    <td width="50%">
...
| ガイド | 分かること |
|--------|------------|
| 🚀 [インストールガイド](docs/ja/installation.md) | uv / pipx インストール、Claude Code / Cursor / Pi / OMP 設定、更新とアンインストール |
