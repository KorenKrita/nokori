...
**前提条件**：Python >= 3.11、Claude Code・Cursor・OMP のいずれかがインストール済み
...
> 詳しいインストールガイド（Claude Code / Cursor / OMP 設定、更新、アンインストール等）は[インストール文書](docs/ja/installation.md)を参照
...
普段どおり Claude Code / Cursor / OMP でコードを書けばよい。ルールにマッチすると、Agent の返信前にリマインドが注入される。`trusted` + `gate_eligible` のルールは、最初の敏感なツール呼び出しを一度差し止める。
...
<table>
  ...
  <tr>
    <td width="50%">
      ...
      Claude Code と Cursor をネイティブにサポートし、OMP は小さな TypeScript ブリッジで同じ Python ディスパッチャへ接続する。
    </td>
    <td width="50%">
...
| ガイド | 分かること |
|--------|------------|
| 🚀 [インストールガイド](docs/ja/installation.md) | pipx インストール、Claude Code / Cursor / OMP 設定、更新とアンインストール |
