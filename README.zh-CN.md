...
**前置条件**：Python ≥ 3.11、已安装 Claude Code、Cursor 或 OMP
...
> 详细安装指南（Claude Code / Cursor / OMP 配置、更新、卸载等）见 [安装文档](docs/zh-CN/installation.md)
...
照常开 Claude Code、Cursor 或 OMP 写代码就行。匹配到规则时，Agent 回复前就能看到注入的提醒；对 `trusted` + `gate_eligible` 的规则，第一次敏感工具调用会被拦一下。
...
<table>
  ...
  <tr>
    <td width="50%">
      ...
      原生支持 Claude Code 与 Cursor，也支持通过一层很薄的 TypeScript bridge 接入 OMP，并复用同一套 Python dispatcher。
    </td>
    <td width="50%">
...
| 指南 | 能帮你了解什么 |
|------|------------------|
| 🚀 [安装指南](docs/zh-CN/installation.md) | pipx 安装、Cursor 配置、更新与卸载 |
