...
旧形态是「抽出一条规则，下次再检索」。新版是一个质量飞轮：规则要先证明自己，才能从记忆变成行为。

| 层 | 亮点 |
|----|------|
| **冷路径规则工厂** | transcript 提取不再是单次总结，而是多角色流水线：extractor、admission judge、rule rewriter、final judge、merge planner、synthetic eval generator、posthoc evaluator。弱规则挡在门外，太宽的规则收窄，不安全的合并会被拒绝或拆分。 |
| **结构化触发器** | 规则带 concepts、required concept groups、trigger variants、excluded contexts、tool tags、severity、source origin、runtime policy version 与 lineage 元数据，不再只是几段松散文本。 |
| **自治生命周期** | 规则从 `candidate → active → trusted`，也可能降为 `suppressed`、靠 shadow 证据恢复，或最终 `archived`。手动命令可以归档，但不能伪造 trust。 |
| **证据回流** | HOT/WARM 注入会产生 fire events；candidate/suppressed 命中会产生 shadow events；SessionEnd 排 posthoc jobs；maintenance 根据评估后的证据自动升降级。 |
| **保守 Gate** | Gate 是最终权限，不是默认行为。它要求 `trusted + gate_eligible`、强 prompt 证据、runtime policy 对得上、prompt hash 新鲜；工具输入可检查时还要 tool-input 证据。 |
| **热路径不调 LLM** | `SessionStart`、`UserPromptSubmit`、`PreToolUse` 只做确定性的检索、匹配、打分、marker I/O 与 fail-open；LLM 工作放到 extract/posthoc 后台任务。 |
| **混合检索** | BM25 永远可用；可选远程 embedding 或本地 Granite 多语言模型补语义召回；RRF 融合排名，再由 v6 applicability 与 MMR 式选择决定 HOT/WARM。 |
| **本地优先** | SQLite、hook 日志、job 队列、Gate marker、embedding 权重、Web UI 状态都在 `~/.nokori/` 下。远程 LLM / embedding 端点是按需启用。 |
| **跨工具支持** | Claude Code 与 Cursor 都支持，包括 Cursor 的 `Shell` 工具名、重复 hook 合并、deferred inject，以及各平台自己的 hook 输出格式。 |
| **可观测性** | `nokori test`、`status`、`health`、`logs`、`extract`、`maintain` 与 Web UI 都能看见规则为什么触发、为什么没触发、证据攒到哪一步、当前配置最终解析成什么。 |

新版最重要的承诺是克制：Nokori 可以早早提醒，但必须攒够证据才有资格变得强势；开始帮忙之后，也要继续接受证据审查。
...
| **promotion**（晋升） | 自治生命周期流转：candidate → active、active → trusted、suppressed 恢复，或跨项目变为 global scope |
| **candidate / active / trusted / suppressed / archived** | v6 生命周期：候选、可注入、已信任、影子恢复、终态归档 |
| **lineage / replacement** | 替换历史存在 lineage / tombstone 里，不再靠运行时 `merged` 状态表示 |
...
- **证据自动激活**：`evidence_score >= 2` 且证据跨了 `>= 2` 个活跃天（含跨项目的 `shadow_hot`），才升 active
...
每次 `UserPromptSubmit`，Nokori 对**正式池 ∪ 影子池**一起做检索（BM25，规则够多时加 embedding 走 RRF），再按池拆开处理：只有正式池的 HOT/WARM 会注入；影子池命中 **HOT 或 WARM** 都只记一笔 `record_shadow_hit`，用于晋升，绝不进当前对话。一条规则被 **≥3 个不同 project_id** 命中过，就升为 `global`（无需再次确认）。`preference` 类规则不参与晋升。
...
新项目即使尚无规则，只要开启 promotion，影子池仍会运行，跨项目共识可从零累积。设置 `NOKORI_PROMOTION_ENABLED=0` 可关闭。
...
若上一场尚未 extract，则从文件**尾部**注入最后 3 条 user 消息（500 字符，预算独立于规则的 1500 字符上限）。**dormant 伪 HOT、shadow 计数、HOT 的 `hit_count`** 均在当轮 **UserPromptSubmit** 写入数据库，不会拖到下次 SessionStart。

**影子池命中与 candidate 激活**：跨项目的 shadow HOT 会记录 `add_evidence(..., shadow_hot, 1)`。若该规则在所属项目中仍为 `candidate`，多日累积的 shadow 命中可计入自动激活条件（`evidence_score >= 2` 且证据跨 `>= 2` 个活跃日）。影子池规则不会注入当前对话，但其命中仍可作为激活证据。
...
权重仅在下列时机下载到 `~/.nokori/models/`，hook 内不下载（以免触发 hook 超时）。从旧的默认模型升级上来后，记得跑一次 `nokori embed prefetch`，并对规则重新索引（`add` / `import` / 或编辑 trigger 相关字段都行），让 `rule_embeddings` 的 `model_version` 跟新模型对齐：
...
注入预算分为两项：规则 1500 字符，热缓存 500 字符（相互独立）。仅**实际写入上下文**的规则会记入 `injections` 并更新 `last_hit` / HOT 的 `hit_count`；因预算截断而未写入的不记录。
...
| 页面 | 内容 |
|------|------|
| **仪表盘** | 规则各状态计数、24h 注入统计、Embed 服务控制（启动/停止）、Gate 状态、待处理提取任务、提升进度 |
| **规则** | 筛选列表、详情页（触发条件、执行动作、证据日志、提升证据、superseded 链）、编辑、退役 |
...
| **生命周期** | 提升进度条（shadow hit 来源项目数 → 全局阈值）、维护任务执行记录 |
...
常见注意点：`[gate] matcher` 只控制 Nokori hook **内部**是否拦截；PreToolUse **是否调用 hook** 由 `~/.claude/settings.json` 决定（见 [Gate 两层匹配](#gate-与-pretooluse两层工具匹配)）。`dismiss_phrase` 的完整说明见 [Dismiss](#4-规则过时了dismiss)。
