...
<p align="center">
  <img src="docs/assets/nokori-one-pager.zh-TW.png" width="880" alt="Nokori 專案一張紙" />
</p>
...
<p align="center">
  <img src="docs/assets/readme-illustrations/01-corrections-become-rules.zh-TW.png" width="880" alt="糾正被沉澱為持久的本地規則" />
  <br />
  <sub>每一次糾正都被沉澱為持久的本地規則。</sub>
</p>
...
<p align="center">
  <img src="docs/assets/readme-illustrations/02-hot-path-recall.zh-TW.png" width="880" alt="熱路徑本地召回，不等待模型" />
  <br />
  <sub>聊天過程中，召回保持本地、確定、無需等模型。</sub>
</p>
...
**前置條件**：Python ≥ 3.11、已安裝 Claude Code、Cursor、Pi 或 OMP 任一個
...
# 註冊 hooks / bridge
nokori install --pi         # 僅 Pi
nokori install --omp        # 僅 OMP
# --all 是 Claude Code + Cursor，--cursor 僅 Cursor，預設只裝 Claude Code

# 驗證（安裝 Pi / OMP 時會顯示 hooks.pi / hooks.omp）
nokori health
```

在 Pi 與 OMP 上，recall 走 `before_agent_start`，Gate 檢查走 `tool_call`，會話結束後的提取由 `session_shutdown` 觸發。Pi 的 `/reload` 會被忽略，不會被 Nokori 誤判為會話結束。
...
> 詳細安裝指南（Claude Code / Cursor / Pi / OMP 設定、更新、解除安裝等）見 [安裝文件](docs/zh-TW/installation.md)
...
照常開 Claude Code / Cursor / Pi / OMP 寫程式就行——匹配到規則時，Agent 回覆前就能看到注入的提醒；對 `trusted` + `gate_eligible` 的規則，第一次敏感工具呼叫會被攔一下。

<p align="center">
  <img src="docs/assets/readme-illustrations/03-gate-stops-risk.zh-TW.png" width="880" alt="Gate 在檔案被觸碰前攔下危險工具呼叫" />
  <br />
  <sub>可信規則可以在危險工具呼叫碰到檔案前先攔一次。</sub>
</p>
...
<table>
  ...
  <tr>
    <td width="50%">
      ...
      原生支援 Claude Code 與 Cursor；Pi 和 OMP 透過小型 TypeScript 橋接接入同一套 Python dispatcher。
    </td>
    <td width="50%">
...
| 指南 | 能幫你了解什麼 |
|------|------------------|
| 🚀 [安裝指南](docs/zh-TW/installation.md) | uv / pipx 安裝、Claude Code / Cursor / Pi / OMP 設定、更新與解除安裝 |
