...
- 信任本地的人：規則躺在你機器上的 SQLite 裡，隨時匯出，整段聊天不外傳
...
聊天時 Nokori 只做檢索和讀寫小檔案，絕不卡你等模型。要動 LLM，得等到關會話之後——它再去 transcript（會話記錄）裡慢慢挖新規矩。
...
| **extract** | 從 transcript 裡用 LLM **提取**候選規矩（冷路徑，不急） |
...
Nokori 在 Claude Code（與 Cursor）裡掛了 **4 個 hook**。你正常聊天時，它們只在本地查庫、算分、讀寫小檔案——**hook 裡絕不調 LLM**，否則每發一條消息都得乾等模型，誰也受不了。
...
# 註冊 hooks。預設只裝 Claude Code；裝了 [local-embed] 會順手 prefetch 權重
...
註冊的 hook 命令是 `python -I -m nokori hook`。`-I` 是隔離模式，忽略 `PYTHONPATH` 和當前目錄，免得你在倉庫根目錄跑 hook 時被本地那個 `nokori/` 原始碼目錄搶了包。日常使用請走 `pip install "nokori[local-embed]"`，只有改 Nokori 自己的原始碼才用 editable 安裝。別指望單靠 `PYTHONPATH` 撐著。
...
**Deferred 注入（`beforeSubmitPrompt` 沒跑時）**：某輪若 Cursor 沒觸發 `beforeSubmitPrompt`，**第一次**匹配的 `preToolUse`（如 `Shell`、`Write`）可能 **deny 一次**，在 `agent_message` 裡帶上完整規則。**deny 後請再執行同一工具**，這是設計如此，不是故障。同輪後面的工具不會再次 deny（按 prompt 原子去重）。
...
**Prompt-hash 不匹配（fail-open）**：`UserPromptSubmit` 寫入 marker 時記錄當前 prompt 的 hash；`PreToolUse` 用 payload 或本 session 最近一條 `injections.prompt_hash` 解析當前 hash（**不會**用磁碟上「最新 marker 檔案」冒充當前輪）。若無法解析或與 marker 不一致（使用者已發下一條消息），**刪除 marker 並放行工具**，不 block。

---

## 自動提取

這是關會話之後才跑的冷路徑，急不來。配好 LLM，Nokori 就去讀那場對話的 **transcript**（`.jsonl` 會話記錄），把你做過的糾正總結成候選規則，再跟庫裡已有的規則做一次合併。整條鏈路都不在交互熱路徑上，慢一點沒人催。
...
**鄰居回填（v0.1 故意保留）**：BM25 預篩不足 5 條鄰居時，會再塞進按 `updated_at` 最近的規則湊到上限一起送 LLM。代價是多耗 token、可能冒出一堆 UNRELATED；換來的是少漏「零詞重疊」的合併。沒有開關。這是有意的取捨：寧可多調幾次 LLM，也不放過本該合併的 SAME/B/D。

---

## 資料庫

規則全躺在一個 SQLite 檔案 `rules.db` 裡，第一次用時自動建好。這個庫跟當前 nokori 版本綁定，換機或升級後要是打不開，先 `nokori export` 備份一份，再換個新的 `NOKORI_DATA_DIR` 或乾脆 `nokori reset`。
...
- **純 AI 證據攢夠**：`evidence_score >= 2` 且證據跨了 `>= 2` 個活躍天（含跨專案的 `shadow_hot`），才升 active
...
每次 `UserPromptSubmit`，Nokori 對**正式池 ∪ 影子池**一起做檢索（BM25，規則夠多時加 embedding 走 RRF），再按池拆開處理：只有正式池的 HOT/WARM 會注入；影子池命中 **HOT 或 WARM** 都只記一筆 `record_shadow_hit`，用於晉升，絕不進當前對話。一條規則被 **≥3 個不同 project_id** 命中過，就升為 `global`（**沒有二次確認**，是 v0.1 的產品取捨）。`preference` 類規則不參與晉升。
...
新專案裡一條規矩都沒有也沒關係，只要開著 promotion，影子池照跑，跨專案共識能從零攢起來。不想要就 `NOKORI_PROMOTION_ENABLED=0` 關掉。

進度在 `nokori status` 裡看得到：`shadow_hits` 和 `N/3 projects=...`。

### Async Extract Mode（關會話後自動挖規矩）

提取預設要你自己動手跑。嫌煩的話，開 async，讓它在會話一關就自己後臺挖：
...
剩下都是些邊角情況的處理，正常用不太會碰到：

- `{data_dir}/extract.lock` 被佔著（另一個實例在跑，或者異常殘留沒清），SessionEnd 就**不**自動開子進程，pending job 留著，回頭手動 `nokori extract` 即可
- SessionEnd 之後 transcript 還在被追加（檔案 `mtime` 變了），`nokori extract` 會**刷新 job 的 mtime、繼續保留 pending**，不會把 job 靜默丟掉
- 損壞到解析不了的 `extract-*.json`，會在 `list_jobs` / `nokori extract` / `SessionStart` 維護時被挪到 `{data_dir}/jobs/bad/`，免得殭屍 job 佔著目錄
...
要是上一場還沒 extract 過，就從檔案**尾部**抓最後 3 條 user 消息注進來（500 chars，獨立預算，不佔那 1500）。順帶說一句：**dormant 偽 HOT、shadow 計數、HOT 的 `hit_count`** 全是在 **UserPromptSubmit 當輪** 就寫庫，不會拖到下次 SessionStart。

**Shadow 餵養 candidate 激活**：跨專案的 shadow HOT 會 `add_evidence(..., shadow_hot, 1)`。如果別的專案那條規則還是 `candidate`，多天累積的 shadow 命中有可能湊夠純 AI 激活線（score ≥ 2 且 2 個活躍日）。這跟「影子池只服務晉升」的直覺不太一樣，但 v0.1 是有意這麼放開的：跨專案檢索證據可以參與激活。
...
- **Prompt ack 清理**：刪超過 24 小時的 `prompt_submit_ack/`、`cursor_deferred/` 檔案；`SessionEnd` 也會順手清掉本 session 的 ack/deferred 目錄
...
怎麼從一堆規矩裡挑出跟你這句話相關的幾條？三步：先用關鍵詞打底（BM25），規則攢多了再疊一層語義向量（embedding），兩份排名用 RRF 揉成一張總榜。最後按 HOT / WARM 檔位決定往上下文裡塞多少字。
...
這裡有兩個都叫「20」的閾值，最容易看混，它們數的根本不是同一批規則：
...
權重在下面這幾個時機才落到 `~/.nokori/models/`，hook 裡從不下載（怕超時）。從舊的預設模型升級上來後，記得跑一次 `nokori embed prefetch`，並對規則重新索引（`add` / `import` / 或編輯 trigger 相關欄位都行），讓 `rule_embeddings` 的 `model_version` 跟新模型對齊：
...
- 一句話原則：hook 裡絕不等模型下載或長時間加載，免得撞上 Claude 的 hook 超時
...
注入預算分兩本帳：規則 1500 chars，熱快取 500 chars（獨立，互不擠佔）。只有**真的寫進了上下文**的規則才記進 `injections` 並更新 `last_hit` / HOT 的 `hit_count`；被預算截掉的那些不記。
...
兩個最容易踩的點：`[gate] matcher` 只管 Nokori hook **內部**攔不攔，而 PreToolUse **要不要呼叫 hook** 是由 `~/.claude/settings.json` 說了算的（見 [Gate 兩層匹配](#gate-與-pretooluse兩層工具匹配)）；`dismiss_phrase` 的完整說明見 [Dismiss](#4-規則過時了dismiss)。
...
關於隱私，幾件事說在前頭：沒有任何網絡同步，資料純本地。規則裡存的是行為描述，不含你的原始碼。只有冷路徑的提取會調 LLM，發出去的也是壓縮後的 transcript 片段，端點指向本地 Ollama 就能徹底離線。

---

## 與現有系統的關係

Nokori 跟你已經在用的那些記憶機制不打架，各管一攤：

| 系統 | 關係 |
|------|------|
| CLAUDE.md | 互補。Nokori 不碰你的 CLAUDE.md；它管的是動態的「遇到 X 就做 Y」 |
| Claude Code auto-memory | 不衝突。memory 偏記事實，Nokori 偏記行為規矩 |
| 其他 memory 外掛 | hook 能共存，但別疊太多「往上下文塞字」的外掛，上下文是有預算的 |
