...
**前置條件**：Python ≥ 3.11、已安裝 Claude Code、Cursor 或 OMP 任一個
...
> 詳細安裝指南（Claude Code / Cursor / OMP 設定、更新、解除安裝等）見 [安裝文件](docs/zh-TW/installation.md)
...
照常開 Claude Code / Cursor / OMP 寫程式就行——匹配到規則時，Agent 回覆前就能看到注入的提醒；對 `trusted` + `gate_eligible` 的規則，第一次敏感工具呼叫會被攔一下。
...
<table>
  ...
  <tr>
    <td width="50%">
      ...
      原生支援 Claude Code 與 Cursor；OMP 透過小型 TypeScript 橋接接入同一套 Python dispatcher。
    </td>
    <td width="50%">
...
| 指南 | 能幫你了解什麼 |
|------|------------------|
| 🚀 [安裝指南](docs/zh-TW/installation.md) | pipx 安裝、Claude Code / Cursor / OMP 設定、更新與解除安裝 |
