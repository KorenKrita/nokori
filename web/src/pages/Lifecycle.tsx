...
interface PromotionData { data: { enabled: boolean; candidates: { short_id: string; project_id: string; trigger_text: string; shadow_hit_count: number; unique_projects: string[]; progress: number; threshold: number }[] } }
    ...
    >
      ...
      <GlassCard>
        ...
        <div className="space-y-2">
          ...
          {Object.keys(maint?.data ?? {}).length === 0 && <p className="text-sm text-text-tertiary">{t('lifecycle.no_maintenance')}</p>}
        </div>
      </GlassCard>
    </motion.div>
  )
