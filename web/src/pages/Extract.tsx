...
export function Extract() {
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      <h2 className="text-2xl font-semibold tracking-tight">Extract Pipeline</h2>

      <div className="grid grid-cols-2 gap-4">
        <GlassCard>
          <h3 className="text-xs font-medium uppercase tracking-wider text-text-tertiary mb-3">
            Pending Jobs ({jobs?.data.pending.length ?? 0})
          </h3>
          {jobs?.data.pending.length === 0 && (
            <p className="text-sm text-text-tertiary">No pending jobs</p>
          )}
        ...
        <GlassCard>
          <h3 className="text-xs font-medium uppercase tracking-wider text-text-tertiary mb-3">
            Done Jobs ({jobs?.data.done.length ?? 0})
          </h3>
          {jobs?.data.done.length === 0 && (
            <p className="text-sm text-text-tertiary">No completed jobs</p>
          )}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-text-tertiary border-b border-white/[0.04]">
              ...
              <th className="text-right py-2">Offset</th>
              <th className="text-left py-2">Extracted</th>
            </tr>
          </thead>
          <tbody>
            {(state?.data ?? []).map((s, i) => (
              <tr key={i} className="border-b border-white/[0.03]">
                <td className="py-2 font-mono text-text-secondary truncate max-w-[300px]">
                  {s.transcript_path}
                </td>
            ...
            {(state?.data ?? []).length === 0 && (
              <tr>
                <td colSpan={4} className="py-4 text-center text-text-tertiary">No extractions</td>
              </tr>
            )}
          </tbody>
        </table>
