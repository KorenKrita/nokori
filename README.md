...
~~当一条 project 规则在 3 个不同项目中被正式池 HOT 注入命中……~~

> **实现变更**：升级证据来自**影子池**，不是当前 session 的正式注入。其他项目 prompt 时独立跑影子 BM25 → `tier_results` → **仅 HOT**（与 §3.2 相同的 top-1 显著性 + 最低证据）才 `record_shadow_hit`。`unique(project_id, date) >= 3` → `project_scope=global`。`preference` 不参与。

### Shadow Pool（影子池）

其他项目的 high-confidence correction/anti_pattern/solution **active** 规则在每次 `UserPromptSubmit` 的 `finally` 中检索（`fetch_shadow_rules`）。

**影子命中**：BM25 top-5 → `tier_results` → **仅 HOT**（与正式池相同：top-1 显著领先 + 最低证据）才 `record_shadow_hit`；不注入当前 session。~~旧文案曾写「不要求 HOT 显著性」~~，已于 2026-05-30 与代码对齐（见 `hooks/user_prompt_submit.py` → `_run_shadow_pool`）。
...
设为 `async` 后，SessionEnd 时会 fork 后台进程执行 `nokori extract`，自动消费待处理的 extract jobs。`NOKORI_EXTRACTING` 仅在 `claude -p` fallback 子进程内设置，用于防止 hook 递归；async 提取子进程本身**不**设置该变量，以便正常调用已配置的 LLM API。spawn 失败不影响 session 关闭。
...
~~条件：rules ≥ 20 且必须配置远程 embedding 端点。~~

> **实现变更**：`rules >= 20` 时若已 `pip install nokori[local-embed]` 也可自动启用本地模型，无需 `NOKORI_EMBED_BASE_URL`。显式 `NOKORI_EMBED_ENABLED=1` 时强制尝试远程或本地。`nokori health` 在规则数 ≥500 且启用 embedding 时给出性能警告。
...
**最低证据**：≥2 个 query token 重叠；或 1 token + trigger variant 命中；或 embedding cosine ≥ 0.55。
