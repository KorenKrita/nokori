...
其他项目的 high-confidence correction/anti_pattern/solution active 规则会在每次 prompt 时被检索（影子池）。**影子命中**指：BM25 top-5 + 最低证据门槛（与正式池相同），**不要求**正式注入用的 HOT 显著性（dominance）——门槛更低，便于跨项目统计。
