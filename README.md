...
其他项目 prompt 时在 `finally` 跑影子池：与正式池相同的 `retrieve_and_tier`（BM25 + 可选 embedding）→ **仅 HOT** 才 `record_shadow_hit`。**≥3 个不同 project_id** 命中后升为 `global`（非「同项目 3 天」）。`preference` 不参与。

### Shadow Pool（影子池）

其他项目的 high-confidence correction/anti_pattern/solution **active** 规则在每次 `UserPromptSubmit` 的 `finally` 中检索（`fetch_shadow_rules`）。

**影子命中**：与正式池相同检索管线（含 embedding RRF，当规则数 ≥20 且已启用时）→ `tier_results` → **仅 HOT** 才计 hit；不注入当前 session。
